function [d] = myImageContrast_ver2(trans,trans0,moving,movingMask,fixed,fixedMask,d,disp,dispi,dispImage)
    % DT := triangulation - not used
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % looks like the edge compare is is not used
    % i am going to attempt to speed up the process by only sampling 
    % the fixed image at the mask points
    % this may affect the edge compare method
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if edgeCompare
    ...
    if DTcompare
        ...
        fixedImageInMovingFrame = bwdist(~(fixedImageInMovingFrame>.8));
    end
