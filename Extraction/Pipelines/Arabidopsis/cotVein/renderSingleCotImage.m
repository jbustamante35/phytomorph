function [] = renderSingleCotImage(dataPoint,toPath)
    try
        ...
        for e = 1:numel(dataPoint)
            ...
            % find the main vein
            if skelPointsBool & branchPointsBool
                %{
                source = pointSets.sPoints(botIdx,:);
                target = pointSets.sPoints(topIdx,:);
                [mainVeinPath] = searchAlongNetwork(Adj,pointSets.sPoints,target,source);
                maPath = pointSets.sPoints(mainVeinPath{1},:);
                plot(maPath(:,2),maPath(:,1),'k.','MarkerSize',3);
                %}
                plot(dataPoint.veinNetwork.mainVein(:,2),dataPoint.veinNetwork.mainVein(:,1),'k.','MarkerSize',3);
                plot(dataPoint.veinNetwork.mainVein(:,2)+offset,dataPoint.veinNetwork.mainVein(:,1),'k.','MarkerSize',3);
            end
