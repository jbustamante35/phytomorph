function [pointListL pointListE SL SE] = minikiniWHOLE(FileList,para)
    try 
        ...
        if isempty(para.TIME);TIME = 1:size(FileList,3);end
        ...
        for t = 1:(numel(TIME)-1)
            ...
            for pt = 1:size(pointListL,1)
                %pointListL(pt,:,t+1) = pointListL(pt,:,t) + fliplr((RES{pt}(end-1:end)));
                pointListE(pt,:,t+1) = fliplr((RES{pt}(end-1:end)));
            end
            ...
            if ~isempty(h)                
                ...
                imshow(I,[])            
                hold on
                %scatter(pointListL(:,2,t),pointListL(:,1,t),'r.');
                scatter(pointListE(:,2,t)+pointListE(:,2,1),pointListE(:,1,t)+pointListE(:,1,1),'r.');
                for k = 1:size(pointList,1)
                    ...
                    %else
                     %   plot(pointListL(k,2,t) + RAD*cos(linspace(-pi,pi,200)),pointListL(k,1,t) + RAD*sin(linspace(-pi,pi,200)),'b');
                    %end
                end
                hold off
                drawnow
