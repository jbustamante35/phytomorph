...
parfor e = 1:numel(FileList)
    videoFile = FileList{e};
    [maskStack(:,:,e),experimentFrame(e),maskStruct(e)] = findDishMask(videoFile,skipN,sigSmoothValue,per,edgeCloseValue,edgeDilateValue,disp);
    if disp
...
for t = 1:(size(trackSlice,4)-1)
    ...
    for pr = 1:size(trackStack,2)
        ...
        nextP = stackSlice(pr).morph2(nextPStatic,domain);
...
for movieSEL = 1:numel(FileList)
    ...
    %}
        ...
        R = regionprops(logical(squareT),'BoundingBox');
        centerImage = imcrop(centerImage,R.BoundingBox);
        spaceChunk = imcrop(spaceChunk,R.BoundingBox);
        timeChunk = imcrop(timeChunk,R.BoundingBox);
        timeChunk_n = imcrop(timeChunk_n,R.BoundingBox);
        ...
        csTOT_view = imcrop(csTOT_view,R.BoundingBox);
