...
close all
iI = double(I)/255;
cpBank = copy(holeGraph.fullSimplePaths);
cpBank.arcLength(100);
samplePaths = cpBank.sampleAlongCurve(0,50,50,iI);
%%
imshow(iI,[]);hold on;
cpBank.plotSampleGrid(0,50,50,iI);
%%
close all
v1 = [];
for e = 1:size(samplePaths,4)
    G = rgb2gray(samplePaths(:,:,:,e));
    [d1,d2] = gradient(G);
    tmp = mean(d1,2);
    v1(:,e) = sort(tmp);
    imshow(G,[]);
...
[Sv,Cv,Uv,Ev,Lv,ERRv,LAMv] = PCA_FIT_FULL_T(v1,2);
plot(Cv(1,:),Cv(2,:),'.')
%% view the graphs
close all
I = imread(dataCluster(5).imageName);
