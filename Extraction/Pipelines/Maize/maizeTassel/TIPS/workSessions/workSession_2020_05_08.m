...
for e = 1:numel(complexIDX)
    ...
    for p = 1:size(tmpbpX,1)
        ...
        for n = 1:numel(nidx)
            ...
            % check if focus point's neighbor is a branch point ?
            % termineal?
            
            tempSkeletonGraph.subsetContains('terminalPoints',curNpoint)
            ...
            % if neighbor is NOT branch point
                ...
                potentialPathsGraph = copy(tempSkeletonGraph);
                
                potentialPathsGraph.removeFromSubset('all',[nHOOD;strPoint]);
                potentialPathsGraph.glue(curNpoint)
                ...
                if ~isinf(D)
                    ...
                    [~,~,pathi] = potentialPathsGraph.shortestpath(1,TARGETS(midx));
