function [] = smartMain_new_ver1(fileName,coneDetNetL_region,coneDetNetR_region,coneDetNetL_fine,coneDetNetR_fine,GMModel,GMModel2,E,U,GMModel_Color,oPath,rPath,disp)
    try
        ...
        for e = 1:numel(fileName)
            ...
            [cropPackage,I] = getSeedlingCropBoxes(fileName{e},coneDetNetL_region,coneDetNetR_region,coneDetNetL_fine,coneDetNetR_fine,oPath,rPath,disp);
            fprintf(['************************************  ENDING STAGE ONE-TWO  ************************************ \n']);
            fprintf(['******************************************************************************************** \n']);
            fprintf(['************************************ STARTING STAGE TWO ************************************ \n']);
            fprintf(['*****************************    image crop mask and skeleton ********************************** \n']);
            LEN = [];
            %[MASK,SKELETON,returnI,boundingBoxes] = cropPackAnalysisAndSave(cropPackage,GMModel,GMModel2);
            [MASK,SKELETON,returnI,boundingBoxes] = getMaskFromCropped(cropPackage,E,U,GMModel_Color);
            ...
            displayResults(fileName{e},double(I),returnI,boundingBoxes,MASK,SKELETON,phenoTypes,oPath,rPath,JSONdoc);
