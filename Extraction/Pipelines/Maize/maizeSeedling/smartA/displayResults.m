function [] = displayResults(fileName,I,returnI,BoundingBoxes,MASK,SKELETON,phenoTypes,oPath,rPath,JSONdoc)
    try
        ...
        if RMSLoutput
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % removed on Feb 13, 2020
            % CyVerse is unable to respond to a download
            % this means that we will have fails due to the server
            % being unable to connet to iRODS@cyverse
            % therefore RMSL is being shut off till I need it.
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % port to rsml
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            plantS = {};
        ...
        for e = 1:numel(returnI)
            fileList{end+1} = [oPath nm '{stage2croppedImage_' num2str(e) '}.tif'];
            imwrite(returnI{e},fileList{end});
        end
        
        for e = 1:numel(returnI)
            fileList{end+1} = [oPath nm '{stage2croppedMaskedImage_' num2str(e) '}.tif'];
            imwrite(MASK{e},fileList{end});
        end
