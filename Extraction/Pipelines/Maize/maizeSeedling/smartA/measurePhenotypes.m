    ...
    try
        ...
        for e = 1:numel(MASK)
            try
                ...
                mP = NaN*zeros(1000,3,numel(path));
                %}
                ...
                for p = 1:numel(path)
                    ...
                    [F{p},theta] = getMidFrame(flip(tmpy,1));
                    tmpy = (F{p}*[flip(tmpy,1) ones(size(tmpy,1),1)]')';


                    tmpy = interp1(1:size(tmpy,1),tmpy,linspace(1,size(tmpy,1),1000));
                    mP(:,:,p) = tmpy;
                    %}

                    tmpy = DP(:,path{p})';
                    %iP = interp1(1:size(tmpy,1),tmpy,linspace(1,size(tmpy,1),1000));
                    %mP = cat(3,mP,iP);
                    %{
                    u_centered_path = bsxfun(@minus,tmpy,mean(tmpy));

                    [DomainS,DomainG] = extendChromosomeMidline(u_centered_path,fliplr(mean(tmpy)),[],1);
                    pathF = ba_interp2(I{e},DomainG(:,:,1),DomainG(:,:,2));
                    imshow(I{e},[]);
                    hold on
                    plot(tmpy(:,2),tmpy(:,1),'r')
                    imshow(pathF,[]);
                    drawnow
                    %}

                    % measure the data along the curve
                    for k = 1:size(I{e},3)
                ...
                %{
                [Sx,Cx,Ux,Ex,Lx,ERRx,LAM] = PCA_FIT_FULL_T(squeeze(mP(:,1,:)),5);
                [Sy,Cy,Uy,Ey,Ly,ERRy,LAM] = PCA_FIT_FULL_T(squeeze(mP(:,2,:)),5);

                figure;
                hold on
                %plot(Uy,-Ux,'g')

                for t = [1 19 30 46]%size(mP,3)
                    tmpy = [mP(:,1,t),mP(:,2,t),ones(size(mP,1),1)];
                    tmpy(any(isnan(tmpy),2),:) = [];
                    tmpy = (inv(F{t})*tmpy')';
                    plot(tmpy(:,2),-tmpy(:,1),'r');

                    if t == 46
                        iF = getMidFrame([tmpy(1,1:2);[tmpy(end,1:2)-50]]);
                        iG = inv(iF);
                    end

                    tmpy = [Sx(:,t),Sy(:,t),ones(size(Sx,1),1)];
                    tmpy(any(isnan(tmpy),2),:) = [];
                    iF = inv(F{t});
                    if t == 46
                        iF = iG;
