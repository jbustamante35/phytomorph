...
for e = 1:size(S,4)
    ...
    tmpImage = S(:,:,:,e);
    %tmpImage = rgb2lab(tmpImage);
    MS(:,:,:,e) = bsxfun(@times,tmpImage,topMask);
    e
end
%%
...
DS = 10;
%DS = 1;
gmL = fitgmdist(initLabels(:,1:DS:end)',4,'Options',options);
...
for i = 1:numel(fidx)
   ...
   %tmp = [viewS(:,:,:,e) simS(:,:,:,e) simS2(:,:,:,e) lab2rgb(simMS(:,:,:,e)) lab2rgb(MS(:,:,:,e))];
    ...
    GOGO = simMC(:,:,:,e);
    GOGO = reshape(GOGO,[size(GOGO,1) 6]);
    [states,prob] = hmm.Viterbi(GOGO',[1 1 1 1 1 1],1);
    
    
    %HMC{i} = [states;GOGO'];
    
    
    plot((((lidx(:,i)-1)==2)*20)+3*61,1:500,'w')
    
     plot((((states-1))*10)+4*61,1:500,'w')
...
kidx = kmeans(h1',nG);
%% try different cluster method - cluster on whole image for noise
toC = squeeze(simC);
szC = size(toC);
...
D(:,find(rm)) = [];
%%
background1 = hmm_node('background1');
background2 = hmm_node('background2');
default1 = hmm_node('default1');
default2 = hmm_node('default2');
centromere = hmm_node('centromere');
foci1 = hmm_node('foci1');
foci2 = hmm_node('foci2');



hmm = my_hmm();
hmm.addNode(background1);
hmm.addNode(background2);
hmm.addNode(default1);
hmm.addNode(default2);
hmm.addNode(centromere);
hmm.addNode(foci1);
hmm.addNode(foci2);
% FIX
hmm.dn = ones(size(initLabels,1),1);
...
backgroundCov = cov(initLabels(:,rawL==BKL )');
backgroundU = mean(initLabels(:,rawL==BKL)');

centromereCov = cov(initLabels(:,rawL==CENL)');
centromereU = mean(initLabels(:,rawL==CENL)');

defaultCov = cov(initLabels(:,rawL==DEL | rawL==DEL)');
defaultU = mean(initLabels(:,rawL==DEL | rawL==DEL)');

fociCov = cov(initLabels(:,rawL==LOL)');
fociU = mean(initLabels(:,rawL==LOL)');



backgroundDis = myProb(backgroundU,backgroundCov);
centromereDis = myProb(centromereU,centromereCov);
defaultDis = myProb(defaultU,defaultCov);
fociDis = myProb(fociU,fociCov);

    
background1.attachDistribution(backgroundDis,1);
background2.attachDistribution(backgroundDis,1);
default1.attachDistribution(defaultDis,1);
default2.attachDistribution(defaultDis,1);
centromere.attachDistribution(centromereDis,1);
foci1.attachDistribution(fociDis,1);
foci2.attachDistribution(fociDis,1);



backGroundTobackGround = constantTransitionFunction(.99);
backGroundTodefault1 = constantTransitionFunction(.01);
default1Todefault1 = constantTransitionFunction(.96);
default1Tofoci1 = constantTransitionFunction(.035);
FF = 20;
foci1Tofoci1 = VarheavisideTransitionFunction(FF,@(x,y)lt(x,y),.9,1);
foci1ToDefault1 = VarheavisideTransitionFunction(FF,@(x,y)ge(x,y),.9,1);
default1Tocen = constantTransitionFunction(.005);
CC = 30;
cenTocen = VarheavisideTransitionFunction(CC,@(x,y)lt(x,y),.9,1);
cenToDefault2 = VarheavisideTransitionFunction(CC,@(x,y)ge(x,y),.9,1);
default2Todefault2 = constantTransitionFunction(.96);
default2Tofoci2 = constantTransitionFunction(.035);
foci2Tofoci2 = VarheavisideTransitionFunction(FF,@(x,y)lt(x,y),.9,1);
foci2ToDefault2 = VarheavisideTransitionFunction(FF,@(x,y)ge(x,y),.9,1);
default2Tobk = constantTransitionFunction(.005);
backGround2TobackGround2 = constantTransitionFunction(1);





background1.attachNode(background1,backGroundTobackGround);
background1.attachNode(default1,backGroundTodefault1);
default1.attachNode(default1,default1Todefault1);
default1.attachNode(foci1,default1Tofoci1);
foci1.attachNode(foci1,foci1Tofoci1);
default1.attachNode(centromere,default1Tocen);
centromere.attachNode(centromere,cenTocen);
centromere.attachNode(default2,cenToDefault2);
default2.attachNode(default2,default2Todefault2);
default2.attachNode(foci2,default2Tofoci2);
foci2.attachNode(foci2,foci2Tofoci2);
foci2.attachNode(default2,foci2ToDefault2);
default2.attachNode(background2,default2Tobk);
background2.attachNode(background2,backGround2TobackGround2);



%[states,prob] = hmm.Viterbi(initLabels(:,1:100),[1 1 1 1]);
