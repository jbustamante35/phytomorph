function [] = DEwrapper(functionName,varargin)
    ...
    if toR('01-Dec-2020')
        ...
        if ~OSG
            webPath = '/de-app-work/';
            sPATH = '//usr/bin/';
            baseDownLoadPath = 'http://data.cyverse.org/dav-anon/iplant/home/nmiller/publicData/';
            ...
            fileShortName = 'dcraw';
            fileNameDownload = [baseDownLoadPath fileShortName];
            %websave([sPATH 'dcraw'],'http://de.cyverse.org/dl/d/B6A77367-7921-42EC-ADAC-1C44EEE51331/dcraw',options);
            websave([sPATH fileShortName],fileNameDownload,options);
            ...
            fileShortName = 'javase-3.2.1.jar';
            fileNameDownload = [baseDownLoadPath fileShortName];
            websave([webPath fileShortName],fileNameDownload,options);
            ...
            fileShortName = 'bioformats_package.jar';
            fileNameDownload = [baseDownLoadPath fileShortName];
            websave([webPath fileShortName],fileNameDownload,options);
            ...
            fileShortName = 'generate_qrcode_R_script.pl';
            fileNameDownload = [baseDownLoadPath fileShortName];
            websave([webPath fileShortName],fileNameDownload,options);
        ...
        if ~isempty(folderfunction)
            ...
            FileList = {};
            FileExt = {'tif','TIF','BMP','bmp','PNG','png','JPG','jpg','tiff','csv','json','czi'};
            ...
            switch folderfunction
                case 'maizeGravi'
                    ...
                    saveName = [pth '.mat'];
                    websave([webPath 'labelData3.mat'],' https://de.cyverse.org/dl/d/6AEED091-1F6F-414F-B745-DD0C34FB1A09/labelData3.mat',options);
                    funcToCall = @(X)cropMaizeImage(X,saveName,'./outout/',0,1,[webPath 'labelData3.mat']);
                case 'midResRoot'
                    % init values
                ...
                case 'waveFront'
                    for e = 1:numel(FileList)
                        ...
                        if (~strcmp(ext,'.czi') && ~strcmp(ext,'.tif')) || contains(p,'masks')
                            rm(e) = true;
                        else
                            rm(e) = false;
                        end
                    ...
                    FileList(rm) = [];
                    toOpOn = FileList{1};
                    fprintf(['Operating wave front on:' toOpOn '\n'])
                    funcToCall = @(X)waveFront(X,'./output/');
            end
        end
        ...
        if ~isempty(filefunction) 
            toOpOn = fileName;
            switch filefunction
                case 'singulatedKernel'
                    funcToCall = @(X)singulatedKernels(X,'./output/');
                case 'RGB_fieldImage'
                    websave([webPath 'fieldRGB_NIR.mat'],' https://de.cyverse.org/dl/d/6DAD39BE-42D1-49FB-91B9-615B8C8231A3/fieldImage_WSU.mat',options);
                    load([webPath 'fieldRGB_NIR.mat']);
                    funcToCall = @(X)obj.func(X,'RGB');
                case 'NIR_fieldImage'
                    websave([webPath 'fieldRGB_NIR.mat'],' https://de.cyverse.org/dl/d/6DAD39BE-42D1-49FB-91B9-615B8C8231A3/fieldImage_WSU.mat',options);
                    load([webPath 'fieldRGB_NIR.mat']);
                    funcToCall = @(X)obj.func(X,'NIR');
                case 'featurePoints'
                ...
                case 'maizeSeedlingV2'
                    ...
                    fprintf(['start:pulling maizeSeedlings_V2.mat\n']);
                    ...
                    funcToCall = obj.func;
                    delete([webPath 'maizeSeedlings_V2.mat']);
                ...
                case 'TIPS2.0'
                    [pth,nm,ext] = fileparts(toOpOn);
                    funcToCall = @(X)TIPS_noBackground(X,['./output/' nm]);
            end 
        end
