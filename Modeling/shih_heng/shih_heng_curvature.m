...
FilePath = '/mnt/tetra/nate/Metfile for Nathan_heatmap/';
FileList = {};
FileExt = {'mat'};
tic
FileList = gdig(FilePath,FileList,FileExt,1);
...
for ee = 1:numel(FileList)
    ...
    try
    %%
    oPath = '/mnt/tetra/nate/curvatureReturn_forPUB/';
    mkdir(oPath)
    close all
    SELECT = 300;
    for root = 1:size(KUR,3)
        ksig = -KUR(SNIP:end-SNIP,:,root);
        %ksig = imfilter(ksig,fspecial('disk',[2]),'replicate');
        figure;
        mesh(ksig(1:SELECT,:));
        view([0 90]);
        axis([0 size(ksig,2) 0 300 -.006 .009])
        %caxis([-.006 .006])
        colormap('jet')
        caxis([-.002 .009])
        view([0 90]);
