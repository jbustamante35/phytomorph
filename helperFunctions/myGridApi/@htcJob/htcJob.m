classdef htcJob < doid
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % a job runs on a node which may-be contained in a collection of nodes
    % a job may-be contained in a colletion of jobs
    % a job may-be a shell job with a main shell
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    properties (Access = private) 
        
        % this can only for this job or factored out over many via bag
        % model
        %resourceList;
       ...
       % for shell file
       curlTemplate = 'curl -H "Pragma:" --speed-limit 1024 --speed-time 30 --retry 3 --retry-delay 6 -o <NAME> <FILE>'
    ...
    methods
        ...
        % constructor
        function [obj] = htcJob(varargin)
            
            % init the list for the job
            obj.resourceList = htcResourceList();
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % init the default requirement
        % X disk, 8 GB ram, 1 cpu
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [] = setMemoryUse(obj,mem)
            if ~ischar(mem);mem = num2str(mem);end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % init the default list of files
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [] = initDefaultTransferList(obj)
            ...
            % REMOVED on April 30, 2020 - using installer as replacement
