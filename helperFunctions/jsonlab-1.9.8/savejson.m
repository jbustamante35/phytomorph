...
elseif(isnumeric(item) || islogical(item))
    %{
        % look at write to disk
        data = 3;
        type = class(data);
        typeWrite = 'double';
        typeRead = 'uint8';
        tid = fopen('~/what.bin','w');
        fwrite(tid,data,typeWrite,'n');
        fclose(tid);
        tid = fopen('~/what.bin','r');
        A = fread(tid,1,typeRead);
    
        % look at write to disk
        data = 3;
        type = class(data);
        tid = fopen('~/what.bin','w');
        fwrite(tid,data,'uint8','n');
        fclose(tid);
        tid = fopen('~/what.bin','r');
        A = fread(tid,1,'uint8=>uint8');
        ...
        fclose(tid);
        tid = fopen('~/what.bin','r');
        A = fread(tid,Inf,'char');
        
        txt2 = jsonencode(item);
