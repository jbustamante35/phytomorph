function [FileData] = parseCyVerseQuery(txt)
    ...
    [uCount] = countOwnership(fileList);
...
function [uCount] = countOwnership(data)
    userUQ = unique({data.USER});
    for e = 1:numel(userUQ)
        uCount(e).count = sum(strcmp({data.USER},userUQ{e}));
        uCount(e).user = userUQ{e};
    end
end
