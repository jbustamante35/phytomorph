...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% my re-write to take a simple version of inputs
if nargin == 2
    if numel(siz) == size(v1,2)
        usize = [1 siz(1:end-1)];
        skipValue = cumprod(usize);
        v1 = v1-1;
        ndx = mtimesx(double(v1),double(skipValue),'T') + 1;
        return
    end
end
...
% assign the first values then skip and hop as needed
ndx = double(v1);
