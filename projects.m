...
dataStoreBase = '/mnt/spaldingdata/nate/octerineDataStore/';

% default project location
projectStoreBase = '/mnt/scratch1/phytomorph_dev/projectStore/';

% added to hardset the userpath 
userpath('/mnt/scratch1/phytomorph_dev/')

% get the process id
sessionPID = feature('getpid');
...
if nargin==0;cmd = 'list';end

% make the new store if it does not exist
if ~exist(projectStoreBase)
    mmkdir(projectStoreBase);
    for e = 1:numel(projectsList)
        if ~strcmp(projectsList(e).ProjectName,'default')
            prj = project.import(projectsList(e));
            project.save(prj);
        end
    end
end
...
switch lower(cmd)
    ...
    case {'open', 'load'}
        ...
        ind = find(strcmpi(prjname, {projectsList.ProjectName}), 1);
        if isempty(ind);error('Projects: unknown project name');end
